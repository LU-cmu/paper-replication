...
#       Reviewers may contact the authors to obtain genus_mapping_PRIVATE2.csv.
# =========================
...
variables <- c("PISA", "BOP", "PESA")


map_file <- "genus_mapping_PRIVATE2.csv"
if (!file.exists(map_file)) {
  stop("Missing mapping file: genus_mapping_PRIVATE2.csv (provide locally; do not upload to GitHub).")
}
...
# Add p for permutation manual）
write_xlsx(
  plot_df,
  path = "plot_df_Spearman_results.xlsx"
)

plot_df <- read_excel("plot_df_Spearman_results1.xlsx")

var_group_levels <- unlist(lapply(
  variables,
...
ggsave(
  "Bubble_Spearman_Pvalue_PISA_Microbiota.pdf",
  plot = p,
  width = 13,
  height = 6
)
